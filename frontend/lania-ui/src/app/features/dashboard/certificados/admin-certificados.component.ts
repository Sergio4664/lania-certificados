...
  /**
   * ✅ NUEVA FUNCIÓN (Lógica del checkbox)
...
    if (tieneCertNormal) {
      // Si ya tiene la normal, forzamos el check y lo deshabilitamos
      conCompetenciasCtrl?.setValue(true);
      conCompetenciasCtrl?.disable();
    } else {
      // Si no tiene la normal, habilitamos el check y lo reseteamos
      conCompetenciasCtrl?.setValue(false);
      conCompetenciasCtrl?.enable();
    }
  }
