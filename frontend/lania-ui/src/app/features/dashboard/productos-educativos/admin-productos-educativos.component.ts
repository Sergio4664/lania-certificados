...
  }
  ...
  /**
   * Inicia el proceso de emisión y envío masivo de constancias para el producto educativo seleccionado.
   * Verifica cuántos participantes no tienen aún una constancia emitida y solicita confirmación.
   * Llama a un método del servicio (que deberás crear) para realizar la operación en el backend.
   */
  emitAndSendCertificates() {
    ...
    // Filtra las inscripciones para encontrar cuántas no tienen un certificado asociado aún.
    const unissuedCount = this.inscripcionesDelProducto.filter(
      insc => !this.getCertificadoForInscripcion(insc.id)
    ).length;

    if (unissuedCount === 0) {
      this.notificationSvc.showInfo('Todas las constancias para este producto ya han sido emitidas.');
      return;
    }

    const confirmationMessage = `Se emitirán y enviarán ${unissuedCount} constancia(s) para el producto "${this.selectedCourse.nombre}". ¿Desea continuar?`;

    if (confirm(confirmationMessage)) {
      this.notificationSvc.showInfo('Iniciando proceso de emisión y envío masivo...');
      
      // NOTA: Deberás crear un método en tu `CertificadoService` que llame al endpoint del backend.
      // Por ejemplo: this.certificadoSvc.emitirYEnviarMasivamente(this.selectedCourse.id)
      console.log(`Llamada al servicio para emitir y enviar constancias para el producto ID: ${this.selectedCourse.id}`);
      
      // **Simulación de la llamada al servicio (reemplazar con la llamada real)**
      // this.certificadoSvc.emitirYEnviarMasivamente(this.selectedCourse.id).subscribe({
      //   next: (response) => {
      //     this.notificationSvc.showSuccess(response.message || 'Proceso completado con éxito.');
      //     // Volver a cargar los datos para reflejar los cambios
      //     this.loadInitialData(); 
      //     this.loadParticipantsForCourse(this.selectedCourse.id);
      //   },
      //   error: (err: HttpErrorResponse) => {
      //     this.notificationSvc.showError(err.error?.detail || 'Ocurrió un error durante la emisión masiva.');
      //   }
      // });
    }
  }
