...
export default class AdminProductosEducativosComponent implements OnInit {
  ...
  loadCertificados() {
    // --- ✅ CORRECCIÓN: Usar 'getAll()' (o el nombre correcto de tu servicio) y añadir tipos ---
    // --- 🚨 VERIFICA que 'getAll' sea el nombre correcto en tu CertificadoService ---
    this.certificadoSvc.getAll().subscribe({ // <-- CAMBIADO DE getAllCertificados a getAll
        next: (certificadosData: Certificado[]) => { // <-- Tipo añadido
        ...
        error: (err: HttpErrorResponse) => { // <-- Tipo añadido
  ...
  issueDocenteCertificate(docenteId: number) { // <-- Argumento quitado
    if (!this.selectedCourse) return;
  ...
  eliminarCertificado(certificadoId: number) {
    if (confirm('¿Está seguro de que desea eliminar esta constancia/reconocimiento? Esta acción no se puede deshacer.')) {
      // --- ✅ CORRECCIÓN: Usar 'delete()' ---
      // --- 🚨 VERIFICA que 'delete' sea el nombre correcto en tu CertificadoService ---
  ...
  emitAndSendCertificates() {
    ...
    if (confirm(`Se intentarán emitir (si faltan) y enviar todas las constancias (NORMALES) para participantes y docentes de "${this.selectedCourse.nombre}". ¿Continuar?`)) {
      ...
      // --- ✅ CORRECCIÓN: Llamar solo con ID ---
      // --- 🚨 VERIFICA que este método en tu servicio/backend emita solo normales por defecto ---
