...
// Interfaces
// --- 💡 CORRECCIÓN 1: Importar 'ProductoEducativoWithDetails' desde su archivo correcto ---
import { 
  ProductoEducativo, 
...
// --- 💡 CORRECCIÓN 1 (continuación): Quitar 'ProductoEducativoWithDetails' de esta importación ---
...
  // --- 💡 CORRECCIÓN 2: Usar el tipo 'ProductoEducativoWithDetails' ---
  private productos: ProductoEducativoWithDetails[] = [];
  docentes: DocenteDTO[] = [];
  participantes: Participante[] = [];
  certificados: Certificado[] = []; // Lista completa de todos los certificados
...
    this.loadCertificados(); // Cargar certificados al inicio
  }
...
      docente_ids: this.fb.array([]), // Será un FormArray
      competencias: [''] // Guardará el string JSON o ''
    }, {
...
  // --- CARGA DE DATOS ---
  loadInitialData() {
    // --- 💡 CORRECCIÓN 4: Guardar ID para re-seleccionar después de recargar ---
    const selectedCourseId = this.selectedCourse?.id; 

    forkJoin({
      // --- 💡 CORRECCIÓN 3: Llamar al servicio que trae los detalles ---
...
      this.groupAndFilterProducts(); // Esto actualiza this.cursos, pildoras, etc.
      
      if (selectedCourseId) {
        // Si había un curso seleccionado, intentar encontrarlo y re-seleccionarlo
        const reselectedCourse = this.productos.find(p => p.id === selectedCourseId);
        if (reselectedCourse) {
          this.selectedCourse = reselectedCourse;
          // No es necesario recargar participantes si ya estaban
        } else {
          this.unselectCourse(); // El curso ya no existe
        }
      }
      
...
              this.cdr.markForCheck(); // Refrescar vista si hay curso seleccionado
            }
        },
...
        // --- 💡 CORRECCIÓN 4: Recargar todos los datos para evitar error de tipos ---
        this.loadInitialData();
...
        // --- 💡 CORRECCIÓN 4: Recargar todos los datos. 'loadInitialData' re-seleccionará el curso ---
        this.loadInitialData();
        this.cancelCourseForm();
      },
...
    if (confirm('¿Está seguro de eliminar este producto educativo? Esto eliminará inscripciones y certificados asociados.')) {
...
          this.cdr.markForCheck();
        },
        error: (err: HttpErrorResponse) => this.notificationSvc.showError(err.error?.detail || 'Error al eliminar. Verifique que no tenga dependencias.')
      });
    }
  }
...
    formArray.clear();
    // 'producto' ahora es 'WithDetails', así que 'docentes' debería estar disponible
...
    if (!docente) { this.notificationSvc.showError("No se encontró al docente."); return; }
...
    if (!this.selectedCourse) { this.notificationSvc.showError('No se ha seleccionado un producto.'); return; }
...
          if (response.errors && response.errors.length > 0) { console.error('Errores en emisión masiva normal:', response.errors); this.notificationSvc.showInfo(`Se encontraron ${response.errors.length} errores. Revise la consola.`); }
          this.loadCertificados(); // Recargar después de acción masiva
         },
        error: (err: HttpErrorResponse) => {
          this.notificationSvc.showError(err.error?.detail || 'Error inesperado durante el proceso masivo (Normal).'); this.loadCertificados();
         }
      });
    }
  }
...
    if (!this.selectedCourse) { this.notificationSvc.showError('No se ha seleccionado un producto.'); return; }
    if (this.selectedCourse.tipo_producto !== 'CURSO_EDUCATIVO') { this.notificationSvc.showError('La emisión de competencias solo aplica a Cursos Educativos.'); return; }
    const selectedParticipantIds = Array.from(this.competencyRecipients);
    if (selectedParticipantIds.length === 0) { this.notificationSvc.showError('No hay participantes seleccionados para emitir competencias.'); return; }
    const inscripcionesAProcesar = this.inscripcionesDelProducto.filter(inscripcion => selectedParticipantIds.includes(inscripcion.participante.id));
    if (!confirm(`Se emitirán y/o enviarán Reconocimientos CON COMPETENCIAS para los ${inscripcionesAProcesar.length} participantes seleccionados de "${this.selectedCourse.nombre}". ¿Continuar?`)) { return; }
...
          catchError(err => { console.error(`Error reenviando a ${inscripcion.participante.nombre_completo}:`, err); this.notificationSvc.showError(`Error reenviando a ${inscripcion.participante.nombre_completo}`); return of({ error: true, inscripcionId: inscripcion.id }); })
        );
      } else {
        console.log(`Creando cert comp para insc ${inscripcion.id}`);
        const payload: CertificadoCreate = { inscripcion_id: inscripcion.id, producto_educativo_id: this.selectedCourse!.id, con_competencias: true };
...
    this.selectedCourse = course; this.competencyRecipients.clear(); this.selectedDocenteIds.clear();
    this.loadParticipantsForCourse(course.id); this.docenteEmailSelection = {};
  }

  unselectCourse() {
    this.selectedCourse = null; this.inscripcionesDelProducto = []; this.showAddParticipantForm = false; this.participantToAdd = null;
  }
...
    // --- 💡 CORRECCIÓN 2: 'filtered' ahora es 'ProductoEducativoWithDetails[]' ---
...
    const isChecked = (event.target as HTMLInputElement).checked; if (isChecked) { this.competencyRecipients.add(participantId); } else { this.competencyRecipients.delete(participantId); }
  }

  toggleAllCompetencyRecipients(event: Event) {
    const isChecked = (event.target as HTMLInputElement).checked; const participantIds = this.courseParticipants.map(p => p.id);
    if (isChecked) { participantIds.forEach(id => this.competencyRecipients.add(id)); } else { this.competencyRecipients.clear(); }
  }
...
    const isChecked = (event.target as HTMLInputElement).checked; if (!this.selectedCourse || !this.selectedCourse.docentes) return; const docenteIds = this.selectedCourse.docentes.map(d => d.id);
    if (isChecked) { docenteIds.forEach(id => this.selectedDocenteIds.add(id)); } else { this.selectedDocenteIds.clear(); }
  }

  isDocenteRecipientSelected = (id: number): boolean => this.selectedDocenteIds.has(id);

  trackByDocenteId(index: number, docente: DocenteDTO): number { return docente.id; }
  trackByInscripcionId(index: number, inscripcion: Inscripcion): number { return inscripcion.id; }
...
          if (Array.isArray(parsed)) { this.competenciesList = parsed.map(String); }
...
    } catch (e) { this.notificationSvc.showError("Error al guardar competencias como JSON."); }
  }

  addCompetency = () => this.competenciesList.push('');
  removeCompetency = (index: number) => { if (this.competenciesList.length > 1) { this.competenciesList.splice(index, 1); } else { this.competenciesList[0] = ''; } }
